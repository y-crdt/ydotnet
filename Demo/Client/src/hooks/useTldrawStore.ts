...
export type UseTldrawStoreOptions = {
    id: string;
    shapeUtils?: TLAnyShapeUtilConstructor[];
};

export function useYjsTldrawStore(opts: UseTldrawStoreOptions) {
    const { yjsDocument, yjsConnector } = useYjs();
    const [store] = useState(() => {
        const store = createTLStore({
            shapeUtils: [...defaultShapeUtils, ...(opts?.shapeUtils || [])],
        });
    ...
    useEffect(() => {
        ...
        function handleSync() {
            ...
            const handleMetaUpdate = () => {
                ...
                if (
                    compareSchemas(theirSchema, store.schema.serialize()) === 1
                ) {
                    window.alert(
